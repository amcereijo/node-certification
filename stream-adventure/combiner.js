...
function maybeWriteBook(stream) {
  if (books !== null) {
    stream.push(`${JSON.stringify(books)}\n`);
  }
  books = null;
}

function write(input, encoding, next) {
  if (input && input.length) {
    const jsonInput = JSON.parse(input);

    if (jsonInput.type === 'genre') {
      maybeWriteBook(this);

      books = { name: jsonInput.name, books: [] };
    } else if (jsonInput.type === 'book') {
      books.books.push(jsonInput.name);
    }
  }
...
const booksStream = through2(write, end);
