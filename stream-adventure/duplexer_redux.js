...
module.exports = function (counter) {
  ...
  function write(input, encoding, next) {
    const counter = counts[input.country] || 0;
    counts[input.country] = counter + 1;
  ...
  const countableStream = through2(write, end);


  return duplexer2({ objectMode: true }, countableStream, counter);
};
